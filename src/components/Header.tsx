...
const Header = () => {
  ...
  return (
    <header className="sticky top-0 z-50 bg-background border-b border-border backdrop-blur-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center py-6">
          ...
          <button 
            ...
            className="md:hidden p-2 rounded-md text-foreground hover:text-primary hover:bg-muted transition-colors duration-200"
      ...
      <div 
        className={`md:hidden fixed top-0 right-0 z-50 h-full w-64 bg-background shadow-lg transform transition-transform duration-300 ease-in-out ${
      ...
      >
        <div className="flex flex-col h-full pt-20 pb-8 px-6">
          <div className="flex-1 space-y-2">
            {navigationLinks.map((link) => (
              <Link
                ...
                className="block py-4 text-lg font-inter font-medium text-foreground hover:text-primary hover:bg-muted px-4 rounded-lg transition-all duration-200 shadow-sm border border-border hover:shadow-md"
              >
                {link.label}
              </Link>
            ))}
          ...
          <div className="pt-6 border-t border-border">
            <Button 
              onClick={() => setIsMobileMenuOpen(false)}
              className="w-full bg-primary hover:bg-primary/90 text-primary-foreground font-inter font-medium py-3 rounded-lg hover:scale-105 transition-all duration-200 shadow-md"
            >
